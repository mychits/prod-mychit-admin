...
const handleEnrollmentRequestPrint = async (id) => {
    ...
    try {
        ...
        doc.setFontSize(12);
        drawTextBox3(`Name: ${enroll?.customer_first_name} ${enroll?.customer_last_name}`, 10, 100);
        drawTextBox(
        ...
        drawTextBox(`PAN No: ${enroll?.customer_pan_no || ""}`, 110, 220);
        ...
        const customerName = `${enroll?.customer_first_name?.replace(/\s+/g, "_")}_${enroll?.customer_last_name?.replace(/\s+/g, "_")}`  || "EnrollmentForm";
